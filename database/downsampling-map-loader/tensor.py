...
if __name__ == '__main__':
    import os
    material_name = '1_spectralon_white_diffuse'
    pbrdf_fn = os.path.join('D:\dataset', material_name, 'table_backward\pbrdf.mat')
    pbrdf_out_fn = os.path.join('D:\dataset', material_name, 'table_backward', material_name + '.pbrdf')
    ...
    # xyzi_T = np.array(pbrdf_dat['xyzi_T'])
    # xyzo_T = np.array(pbrdf_dat['xyzo_T'])
    # pbrdf_T = np.array(pbrdf_dat['pbrdf_T'])
    # pbrdf_T_f = np.array(pbrdf_dat['pbrdf_T_f'])

    #write_tensor(filename=pbrdf_out_fn, phi_d=phi_d, theta_d=theta_d, theta_h=theta_h, xyzi_T=xyzi_T, xyzo_T=xyzo_T, pbrdf_T=pbrdf_T, pbrdf_T_f=pbrdf_T_f)
    write_tensor(pbrdf_out_fn, **pbrdf_dat)
